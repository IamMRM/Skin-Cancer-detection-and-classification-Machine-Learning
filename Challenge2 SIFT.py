...
mask, img_rgb, image, output, grayScale = helper.loading(9,file,bcc,bcc_seg)
...
keypoints_1, descriptors_1 = sift.detectAndCompute(image,mask)
...
for i,nam in enumerate(file):
    mask, img_rgb, __, __, __ = helper.loading(i,file,bcc,bcc_seg,zoom=0)
...
for i,nam in enumerate(file):
    mask, img_rgb, __, __, __ = helper.loading(i,file,bkl,bkl_seg,zoom=0)
    keypoints_1, descriptors_1 = sift.detectAndCompute(img_rgb,mask)
    if descriptors_1 is not None:
...
for i,nam in enumerate(file):
    mask, img_rgb, __, __, __ = helper.loading(i,file,mel,mel_seg,zoom=0)
    keypoints_1, descriptors_1 = sift.detectAndCompute(img_rgb,mask)
    if descriptors_1 is not None:
